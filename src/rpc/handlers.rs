...
use super::types::*;
use crate::rpc::RpcState; // Import RpcState
...
pub async fn handle_rpc_request(
    ...
    let result = match req.method.as_str() {
        ...
        "getTransactionStatus" => handle_get_transaction_status(state.chain.clone(), req.params).await,
    ...
    match result {
        Ok(_) => {
            ...
            let msg = crate::network::NetMessage::SubmitTx(
                crate::network::TransactionPayload::Mint(tx)
            );
...
async fn handle_submit_burn(
    ...
    match result {
        Ok(_) => {
            let msg = crate::network::NetMessage::SubmitTx(
                 crate::network::TransactionPayload::Burn(tx)
            );
            crate::network::broadcast_message(state.peer_manager.clone(), msg).await;
...
async fn handle_submit_transaction(
    ...
    let (tx_hash, result) = {
        ...
        if tx.nonce != sender_nonce + 1 {
             ...
             return Err(RpcError {
                code: -32000,
                message: format!("Invalid nonce: expected {}, got {}", sender_nonce + 1, tx.nonce),
            });
        }
